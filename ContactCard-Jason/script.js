...
document.getElementById("save-contact")?.addEventListener("click", () => {
  openFile("Jason-Mizrahi.vcf", "Jason-Mizrahi.vcf");
});

document.getElementById("share-contact")?.addEventListener("click", async () => {
  const response = await fetch("Jason-Mizrahi.vcf");
  const blob = await response.blob();
  const file = new File([blob], "Jason-Mizrahi.vcf", {
  ...
  if (navigator.canShare && navigator.canShare({ files: [file] }) && navigator.share) {
    await navigator.share({
      title: "Jason Mizrahi",
      text: "Jason Mizrahi contact card for Champion Auto Finance",
      files: [file],
    });
    return;
  }
